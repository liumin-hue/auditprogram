      ...
      <mt-header fixed title="工单改派">
        ...
        <div class="reassign_content">
          ...
          <div class="mui-table-view-cell" @click="choosePerson()">
            <div>处理人</div>
            <span>{{disposeReason}}<span class="mui-icon mui-icon-arrowdown"></span></span>
          </div>
...
export default {
  ...
  activated() {
    ...
    if (pageSource !== "workOrderReplyResult") {
      this.$http
        .get(
          this.$myConfig.host +
            "/api/WorkBillChangeDispatch/GetWorkBillChangeDispatchUsers",
          {
            params: {
  ...
  methods: {
    ...
    onValuesChange(picker, values) {
      if (values[0]) {
        this.disposeReason = values[0].RealName;
        this.dealUserId = values[0].UserID;
      }
    },
    //点击保存按钮
    save() {
      MessageBox.confirm("确定改派工单")
        .then(action => {
          this.$http
            .post(
              this.$myConfig.host +
                "/api/WorkBillChangeDispatch/PostWorkBillChangeDispatch",
              {
