<template>
  <div id="big-box">
      <mt-header fixed :title="CommunityName" id="GIS_community">
    ...
    <div class="nav clearfix">
      <span class="tab_btn">
        <div id="wait_for_reply" size="small" @click="wait_for_reply()" :class="active == 'wait_for_reply' ? 'checked' : '' ">工单池</div>
        <div id="reply_record" size="small" @click="reply_record()" :class="active == 'reply_record' ? 'checked' : '' ">待回复</div>
      </span>
    </div>
      <!-- 工单内容 -->
...
export default {
  data() {
    return {
      ...
      hostPath:"/api/RepairUserTrajectory/GetGISCommunityWorkOrderPoolListByArriveDate",//路由路径
    };
  },
  //监听变化
  watch: {
  ...
  activated() {
    ...
    //判断前一页是否为home
    if(fromPath == "Community" || fromPath == "Home"){
      // 初始化数据
      this.confirms = []
      this.replys = []
      ...
      this.isLoading = true
      //刷新页面
      this.hostPath = "/api/RepairUserTrajectory/GetGISCommunityWorkOrderPoolListByArriveDate";
      this.reply_record()
      this.wait_for_reply();
    }
  },
  ...
  methods: {
    ...
    this.$store.lng = this.$route.params.lng
    this.$store.lat = this.$route.params.lat
    },
    //下拉刷新
    ...
    reply_loadTop: function() {
      let hostPath = "/api/RepairUserTrajectory/GetGISCommunityWaitReplyBillsByArriveDate";
